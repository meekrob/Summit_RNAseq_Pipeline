...
outhisat2=$(realpath ${outputdir}03_hisat2)
mkdir -p $outhisat2
...
cmd4="$featureCounts -p -Q 20 -T ${pthread} -a $gtffile -o ${outfeature}counts_$samplename.txt ${samfilePath[*]}"
echo -e "\n \t $ $cmd4"
time eval $cmd4
# quit on fail
if [ $? -ne 0 ]
...
do
    ...
    then
        fail_status=$?
        echo "============================== samtools view FAILED on ${seqname} ====================================" 1>&2;
        exit $fail_status
    fi
    ...
    then
        fail_status=$?
        echo "==================================== samtools sort FAILED on ${seqname} ====================================" 1>&2;
        exit $fail_status
    fi
