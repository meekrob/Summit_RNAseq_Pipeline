...
gtffile="/projects/dcking@colostate.edu/support_data/hg38/hg38Annotation"
...
fastp="singularity exec --bind /scratch/summit/$USER:/tmp $container fastp"
...
#DATE='2018-10-16'
#SCRATCHDIR=$(readlink ../03_output_scratch)
#outputdir="$SCRATCHDIR/${DATE}_RRoutput/"
outputdir="../03_output_scratch/"$DATE"_RRoutput/"
...
do
 ...
 do
  ...
  echo -e "\n FASTP quality reports are located in:\n $fastpReport"
    ...
    ## execute fastp
    cmd1="$fastp -i ${inputdir}${read1}${suffix} -I ${inputdir}${read2}${suffix} -o ${fastpTrim}${read1}_trim${suffix}.gz -O ${fastpTrim}${read2}_trim${suffix}.gz --json=${fastpReport}${read1}_report.json --html=${fastpReport}${read1}_report.html -g -x -p -V -w $pthread"
    ...
    ls -l ${fastpTrim}${read1}_trim${suffix}.gz
    check_fail $? 'ls 1'
    ls -l ${fastpTrim}${read2}_trim${suffix}.gz
    check_fail $? 'ls 2'
