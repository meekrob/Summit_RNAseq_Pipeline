...
do
  ...
  echo -e "\n FASTP quality reports are located in:\n $fastpReport"
    ...
    ## execute fastp
    cmd1="$fastp -i ${inputdir}${sample1}${suffix} -o ${fastpTrim}${sample1}_trim${suffix} --json=${fastpReport}${sample1}_report.json --html=${fastpReport}${sample1}_report.html"
    echo -e "\t $ ${cmd1}"
    time eval $cmd1
done
...
do
  ...
  sample1=${samples[$counter]}_trim
  echo "FASTQ File: $sample1"
  fastpTrim=$outputdir"02_fastp_trim/"$samplename/
  echo "FASTQ Path: ${fastpTrim}${sample1}${suffix}"
  ## execute hisat2
...
do
    samfile=${samples[$counter]}_trim.sam
    echo "File: $samfile"
